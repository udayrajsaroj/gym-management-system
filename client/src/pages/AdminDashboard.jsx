...
------------------------------------------
      ...
      alert(`Error: ${err.response?.data?.message || "Stats route not found on Backend."}`);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Permanent Action: Remove this user?")) {
      try {
    ...
    <div className="min-h-screen bg-[#0f172a] text-white p-4 md:p-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          ...
          <Link to="/admin/attendance" className="bg-white/5 border border-white/10 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all flex items-center gap-2">
                ...
                <tr key={user._id} className="hover:bg-white/[0.03] transition-all group">
                  ...
                  <td className="p-6 text-right">
                    <div className="flex justify-end gap-2">
                      {user.role === 'member' && (
                        <button onClick={() => handleViewStats(user._id, user.name)} className="p-3 bg-white/5 hover:bg-blue-500 rounded-xl transition-all" title="View Attendance Analytics">
      ...
      {isQRModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-12 flex flex-col items-center shadow-2xl">
...
const StatCard = ({ title, value, color }) => (
  <div className="bg-white/5 p-8 rounded-[2rem] border border-white/10 relative overflow-hidden group">
    ...
    <div className="flex items-end gap-3"><h3 className="text-5xl font-black tracking-tighter leading-none">{value}</h3><div className={`w-2 h-2 rounded-full mb-2 animate-pulse ${color}`}></div></div>
  </div>
);
